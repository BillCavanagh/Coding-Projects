...
public class Pawn extends DefaultPiece {
    ...
    public HashSet<Move> getPossibleMoves(Board board) {
        ...
        if(board.checkAvailable(forwardOne.row,col) && board.getPiece(forwardOne.row,col) == null){ // check if the space ahead of the pawn is free and not a piece
            possibleMoves.add(forwardOne);
            if (!hasMoved){ // if the pawn hasnt moved
                if(board.checkAvailable(forwardTwo.row, col) && board.getPiece(forwardTwo.row,col) == null){ // check two spaces ahead
